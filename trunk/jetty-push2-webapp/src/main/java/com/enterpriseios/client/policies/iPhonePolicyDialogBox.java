 ...
 */
public class iPhonePolicyDialogBox extends DialogBox implements ClickHandler, PolicyDialogBox
{

    @UiTemplate("iPhonePolicyDialogPanel.ui.xml")
    interface Binder extends UiBinder<Widget, iPhonePolicyDialogBox>
    {
    }
    ...
    @UiField(provided=true)
    TogglePolicy allowCamera;

    @UiField(provided=true)
    TogglePolicy allowBrowser;
    ...
    private Timer timer;
    private static Logger logger = Logger.getLogger("iPhonePolicyDialogBox");

    public iPhonePolicyDialogBox(DeviceController controller,boolean autoHide,boolean modal)
    {                                                                      
        ...
        maxInactivityTimeDeviceLock=new DropBoxPolicy(mediator,"MaxInactivityTimeDeviceLock",new int[][]{{1,60},{2,120},{3,180},{4,240},{5,300}});
        devicePasswordHistory=new NumericPolicy(mediator,"DevicePasswordHistory",3);
        allowCamera=new TogglePolicy(mediator,"AllowCamera");
        allowBrowser=new TogglePolicy(mediator,"AllowBrowser");
        ...
        remoteWipe.addValueChangeHandler(new ValueChangeHandler<Boolean>()
        {
            ...
            public void onValueChange(ValueChangeEvent<Boolean> changeEvent)
            {
                ...
                if(checked)
                {
                    image.setUrl("iPhone_wipe.png");
                    mediator.setEnabled(false);
                }
                else
                {
                    image.setUrl("iPhone_top.png");
